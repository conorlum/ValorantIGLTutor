	...
	this._finalizeConsent = function(type, consent, isGdprApproved) {
		...
		if (type == 'gdpr') {
			if (consent) {
				...
				this.url += (isGdprApproved ? '&isWePassGdpr=1' : '&isWePassGdpr=0') + '&gdprConsent=' + consent + '&gppConsent=&gppSid=';
				if (isGdprApproved) {
					this._getUuid();
				}
			}
			...
			this.gppResolve();
				...
				var urlDetObj = new SekindoClientDetections_URL({
					url: 'https://live.primis.tech/live/liveView.php?s=108077&debugInformation=ABT+%2F+GPPServerSide+%2F+3+%2F+active&x=SEKXLEN&y=SEKYLEN&cbuster=1777497676',
					origQString: 's=108077&debugInformation=ABT+%2F+GPPServerSide+%2F+3+%2F+active',
					placementId: 108077,
					x: 600,
					y: 400,
					videoType: 'sticky',
					...
					geo: 'US',
					dmaCode: 825,
					dynamicSetup: [["clientInfoSetup",[{"deviceType":"smartphone","os":"ios","osVer":"14"},{"floatConfig":{"width":"213","height":"120","direction":"br","verticalOffset":"94","horizontalOffset":"1","isCloseBtn":1,"closeBtnPos":"right"}}]]],
					uuid: '69db455987495',
